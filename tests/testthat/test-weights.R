...
gen_weighted_binary <- function(seed = 77) {
  set.seed(seed)
  n <- 1000; x <- cbind(1, rnorm(n)); z <- rbinom(n, 1, 0.4)
  psi0 <- c(0.8, -0.5, 0.7)
...
test_that("mcglm errors on wrong-length freq_weights", {
  ...
  expect_error(mcglm(d$y, d$z_hat, d$x, family = "poisson", method = "naive",
                     freq_weights = rep(1, 5)), "freq_weights must have length")
})
...
test_that("freq_weights=2 on all obs equals fitting on doubled data (binary naive)", {
  set.seed(88); n <- 200; x <- cbind(1, rnorm(n))
  ...
  fit_dup <- mcglm(rep(y, 2), rep(z_hat, 2), rbind(x, x),
...
test_that("freq_weights with duplication equivalence for BCA/BCM/CS (binary)", {
  set.seed(89); n <- 300; x <- cbind(1, rnorm(n))
  z <- rbinom(n, 1, 0.4); p01 <- 0.1; p10 <- 0.15; pi_z <- 0.4
  z_hat <- z
  z_hat[z == 0] <- rbinom(sum(z == 0), 1, p01)
  z_hat[z == 1] <- 1 - rbinom(sum(z == 1), 1, p10)
  y <- rpois(n, exp(0.8 * z + x %*% c(-0.5, 0.7)))
  wt <- rep(1, n); wt[1:50] <- 2

  fit_wt <- mcglm(y, z_hat, x, family = "poisson",
                   method = c("naive", "bca", "bcm", "cs"),
                   p01 = p01, p10 = p10, pi_z = pi_z, freq_weights = wt)
  idx <- c(1:n, 1:50)
...
test_that("unit freq_weights produce identical results to no weights (multi)", {
  set.seed(90); n <- 500; K <- 3; pi_z <- c(0.5, 0.3, 0.2)
  ...
  x <- cbind(1, rnorm(n))
  y <- rpois(n, exp(as.numeric(x %*% c(0.8, -0.7)) + c(0, 1.0, -0.9)[z + 1]))
  ...
  fit_unit  <- mcglm(y, z_hat, x, family = "poisson",
                      method = c("naive", "bca", "bcm", "cs"),
                      Pi = Pi, pi_z = pi_z, freq_weights = rep(1, n))
  for (m in c("naive", "bca", "bcm", "cs")) {
    expect_equal(coef(fit_unit, m), coef(fit_no_wt, m), tolerance = 1e-10,
...
test_that("freq_weights duplication equivalence for multicategory", {
  set.seed(91); n <- 300; K <- 3; pi_z <- c(0.5, 0.3, 0.2)
  z <- sample(0:(K - 1), n, replace = TRUE, prob = pi_z)
  Pi <- matrix(c(0.8, 0.1, 0.1, 0.1, 0.8, 0.1, 0.1, 0.1, 0.8),
               nrow = K, byrow = TRUE)
  z_hat <- integer(n)
...
test_that("compute_mhat_bin with unit weights matches unweighted", {
  set.seed(100); n <- 200; x <- cbind(1, rnorm(n)); psi <- c(0.8, -0.5, 0.7)
  fam <- mcGLM:::get_link_funs("poisson")
  m1 <- mcGLM:::compute_mhat_bin(psi, x, fam$mu, 0.1, 0.15, 0.4)
  m2 <- mcGLM:::compute_mhat_bin(psi, x, fam$mu, 0.1, 0.15, 0.4, wt = rep(1, n))
  expect_equal(m1, m2, tolerance = 1e-12)
})

test_that("compute_Ihat with unit weights matches unweighted", {
  set.seed(101); n <- 200; x <- cbind(1, rnorm(n))
  z_hat <- rbinom(n, 1, 0.5); psi <- c(0.8, -0.5, 0.7)
  fam <- mcGLM:::get_link_funs("poisson")
  xi_hat <- cbind(z_hat, x)
  I1 <- mcGLM:::compute_Ihat(psi, xi_hat, fam$mu_dot)
  I2 <- mcGLM:::compute_Ihat(psi, xi_hat, fam$mu_dot, wt = rep(1, n))
  expect_equal(I1, I2, tolerance = 1e-12)
})

test_that("compute_Mhat_bin with unit weights matches unweighted", {
  set.seed(102); n <- 200; x <- cbind(1, rnorm(n)); psi <- c(0.8, -0.5, 0.7)
  fam <- mcGLM:::get_link_funs("poisson")
  M1 <- mcGLM:::compute_Mhat_bin(psi, x, fam$mu, fam$mu_dot, 0.1, 0.15, 0.4)
  M2 <- mcGLM:::compute_Mhat_bin(psi, x, fam$mu, fam$mu_dot, 0.1, 0.15, 0.4,
                                  wt = rep(1, n))
...
test_that("vcov_naive with unit weights matches unweighted", {
  ...
  V1 <- mcGLM:::vcov_naive(psi, d$y, d$xi_hat, "poisson")
  V2 <- mcGLM:::vcov_naive(psi, d$y, d$xi_hat, "poisson", wt = rep(1, d$n))
...
test_that("vcov_cs_bin with unit weights matches unweighted", {
  ...
  V1 <- mcGLM:::vcov_cs_bin(psi_cs, d$y, d$xi_hat, d$x, "poisson",
                             d$p01, d$p10, d$pi_z)
  V2 <- mcGLM:::vcov_cs_bin(psi_cs, d$y, d$xi_hat, d$x, "poisson",
...
test_that("vcov_bc_bin corrected with unit weights matches unweighted", {
  ...
  psi_bca <- mcGLM:::fit_bca_bin(psi_naive, d$y, d$xi_hat, d$x, "poisson",
                                  d$p01, d$p10, d$pi_z)
  V1 <- mcGLM:::vcov_bc_bin(psi_bca, d$y, d$xi_hat, d$x, "poisson",
                             d$p01, d$p10, d$pi_z, psi_naive = psi_naive,
                             type = "bca", corrected = TRUE)
  V2 <- mcGLM:::vcov_bc_bin(psi_bca, d$y, d$xi_hat, d$x, "poisson",
                             d$p01, d$p10, d$pi_z, psi_naive = psi_naive,
                             type = "bca", corrected = TRUE, wt = rep(1, d$n))
  expect_equal(V1, V2, tolerance = 1e-10)
})

test_that("vcov_naive with weights=2 matches duplicated data", {
  set.seed(103); n <- 200; x <- cbind(1, rnorm(n))
  z_hat <- rbinom(n, 1, 0.5)
  y <- rpois(n, exp(0.5 * z_hat + x %*% c(-0.3, 0.4)))
  xi_hat <- cbind(z_hat, x)
  naive_wt <- mcGLM:::fit_naive_bin(y, xi_hat, "poisson", wt = rep(2, n))
  V_wt <- mcGLM:::vcov_naive(naive_wt$coefficients, y, xi_hat, "poisson",
                              wt = rep(2, n))
  xi2 <- rbind(xi_hat, xi_hat)
  naive_dup <- mcGLM:::fit_naive_bin(rep(y, 2), xi2, "poisson")
  V_dup <- mcGLM:::vcov_naive(naive_dup$coefficients, rep(y, 2), xi2, "poisson")
...
test_that("fractional freq_weights are accepted and produce finite results", {
  ...
  fit <- mcglm(d$y, d$z_hat, d$x, family = "poisson",
               method = c("naive", "bca", "bcm", "cs"),
               p01 = d$p01, p10 = d$p10, pi_z = d$pi_z, freq_weights = wt)
  for (m in c("naive", "bca", "bcm", "cs"))
    expect_true(all(is.finite(coef(fit, m))), info = paste("method:", m))
})

test_that("freq_weights work with binomial family", {
  set.seed(104); n <- 500; x <- cbind(1, rnorm(n))
  z <- rbinom(n, 1, 0.4); psi0 <- c(0.8, -0.5, 0.7)
  y <- rbinom(n, 1, plogis(psi0[1] * z + x %*% psi0[-1]))
  p01 <- 0.10; p10 <- 0.15; pi_z <- 0.4
  z_hat <- z
  z_hat[z == 0] <- rbinom(sum(z == 0), 1, p01)
  z_hat[z == 1] <- 1 - rbinom(sum(z == 1), 1, p10)
  ...
  fit2 <- mcglm(y, z_hat, x, family = "binomial",
                method = c("naive", "bca", "bcm", "cs"),
                p01 = p01, p10 = p10, pi_z = pi_z, freq_weights = rep(1, n))
  for (m in c("naive", "bca", "bcm", "cs"))
